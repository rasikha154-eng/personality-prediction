...
const TestInterface = () => {
  ...
  const [expressionCountdown, setExpressionCountdown] = useState<number>(0);
  ...
  useEffect(() => {
    if (voiceResults) console.log("Voice results:", voiceResults);
  }, [voiceResults]);
  ...
  const analyzeTextResponse = async () => {
    ...
    try {
      ...
      if (textData && !textData.error) {
        ...
        setTextAnalysisComplete(false);
        toast({ title: "❌ Text Analysis Failed", description: textData.error || "Could not process.", variant: "destructive" });
        return false;
      }
    } catch {
      setTextAnalysisComplete(false);
      toast({ title: "❌ Text Analysis Failed", description: "Could not process.", variant: "destructive" });
      return false;
    } finally {
        ...
        recorder.onstop = async () => {
          ...
          try {
            ...
            if (data.error) {
              ...
              setVoiceResults(data.voice || data);
    ...
    if (!isCameraActive) {
      try {
        ...
        if (videoRef.current) {
          ...
          await new Promise<void>((resolve) => { videoRef.current!.onloadedmetadata = () => resolve(); });
  ...
  // ── Expression sequence with countdown ────────────────────────────────────
  const startExpressionSequence = () => {
    let expressionIndex = 0;

    const processNextExpression = async () => {
          ...
          if (blob) {
            ...
            try {
              ...
              toast({
                ...
                description: `${expr.label} — ${data?.dominant_emotion || "processed"}`,
              });
            } catch {
      ...
      setIsCapturing(false);
      expressionIndex++;
      setTimeout(() => processNextExpression(), 1000);
    };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (fusionResults.error) {
        ...
        if (fusionResults.fusion) localStorage.setItem("tv_fusion_results", JSON.stringify(fusionResults.fusion));
        ...
        <Card className="bg-card border border-border p-8 shadow-lg">
          ...
          {currentStep === 1 && (
            <div className="text-center">
              ...
              <div className="mb-8 space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 bg-secondary/20 border border-border rounded-lg p-6">
                  <div className="space-y-2">
                    ...
                    <Input id="userName" type="text" placeholder="Enter your name" value={userName}
                  ...
                  <div className="space-y-2">
                    ...
                    <Input id="userAge" type="number" placeholder="Enter your age" min="1" max="120" value={userAge}
              ...
              <div className="mb-8 space-y-4">
                <p className="text-gray-300 font-lora mb-4">Now, please answer these questions:</p>
            ...
            <div className="text-center">
              ...
              <div className="flex flex-col items-center">
                <div className="relative">
                  <Button onClick={toggleRecording} size="lg" disabled={isVoiceLoading}
                    className={`rounded-full w-20 h-20 mb-4 transition-all duration-300 ${
                      isVoiceLoading ? "bg-accent/50 cursor-not-allowed"
                      : isRecording ? "bg-red-500 hover:bg-red-600 animate-pulse"
                      : "bg-accent hover:bg-accent/80"
                    } text-white`}>
                    {isVoiceLoading ? <Loader2 className="h-8 w-8 animate-spin" />
                ...
                <p className="text-gray-300 mb-2">
                  {isVoiceLoading ? "Microphone initialize ho raha hai..."
                ...
                {isVoiceLoading && (
                  <div className="w-48 mt-3">
                    ...
                    <p className="text-xs text-gray-400 mt-2 text-center">Loading...</p>
                  </div>
                )}

                {!isRecording && !recordingComplete && audioBlob && (
                  <div className="flex items-center gap-2 mt-3 text-accent">
                    <Loader2 className="h-4 w-4 animate-spin" />
                    <span className="text-sm">Voice analyze ho raha hai...</span>
                  </div>
                )}
                ...
                {isCameraActive ? (
                  <div className="relative w-full aspect-video bg-black rounded-lg overflow-hidden">
                    <video ref={videoRef} autoPlay muted playsInline
                      ...
                      onLoadedMetadata={(e) => { (e.target as HTMLVideoElement).play().catch(console.error); }}
                    ...
                    {/* Expression guidance with countdown */}
                          ...
                          {expressionCountdown > 0 ? (
                            <div className="flex flex-col items-center">
                              <div className={`w-12 h-12 rounded-full border-4 flex items-center justify-center font-bold text-xl transition-all duration-300 ${
                                expressionCountdown <= 2 ? "border-red-400 text-red-400 scale-110" : "border-accent text-accent"
                              }`}>
                                {expressionCountdown}
                              </div>
                            ...
                            <div className="flex flex-col items-center">
                              ...
                              <p className="text-xs text-gray-400 mt-1">capturing</p>
                            </div>
                          ) : expressionComplete ? (
                      ...
                      {expressionCountdown > 0 && (
                        <div className="mt-2 h-1 bg-white/10 rounded-full overflow-hidden">
                          <div className="h-full bg-accent rounded-full transition-all duration-1000"
                            style={{ width: `${(expressionCountdown / 5) * 100}%` }} />
                        </div>
                      )}
                    </div>
